...
class get_video:
    ...
    def get_cid_ang_part(self):
       req1=requests.get(url="https://api.bilibili.com/x/player/pagelist?aid={}&jsonp=jsonp".format(self.AV),headers=headers)
       rep1=json.loads(req1.text)
       cid=rep1.get("data")[page-1]["cid"]
       part=rep1.get("data")[page-1]["part"]
       self.cid=cid
       self.part=part

    def get_and_save_flv_url(self,qua):
        a=requests.get(url="https://api.bilibili.com/x/player/playurl?avid={}&cid={}&qn={}&type=&otype=json".format(self.AV,self.cid,qua),headers=headers)
        c=json.loads(a.text)
        b=c["data"]["durl"][0]["url"]
        headers["referer"]="https://www.bilibili.com/av{}".format(self.AV)
...
if __name__=="__main__":
    try:
        ...
        page=int(input("请选择下载哪一P："))
        qua=int(input("请选择清晰度(请输入数字，16为360p,32为480p,64为720p,80为1080P)："))
    except ValueError:
        print("您输入的不是数字，请重试。")
        BV=input('请输入BV号(例如：BV1764y1975D )：')
        page=int(input("请选择下载哪一P："))
    ...
    start=get_video(BV)
    start.BvToAv()
    start.get_cid_ang_part()
    start.get_and_save_flv_url(qua)
    print("下载完成")
    time.sleep(3)